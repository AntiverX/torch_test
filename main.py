...
def main():
    ...
    def criterion(x, x_):
        return criterion1(x, x_) + criterion2(x, x_)
...
def train(train_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        if epoch % 1 == 0 and i % opt.resultPicFrequency == 0:
            # Antiver
            rev_secret_img_ = Rnet(cover_img)
            # end Antiver
            save_result_pic(this_batch_size, cover_img, container_img.data, secret_img, rev_secret_img.data, rev_secret_img_.data, epoch, i, opt.trainpics)
...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        rev_secret_img_ = Rnet(cover_img)
        # end Antiver
        save_result_pic(this_batch_size, cover_img, container_img.data, secret_img, rev_secret_img.data, rev_secret_img_.data, epoch, i, opt.validationpics)
...
def save_result_pic(this_batch_size, originalLabelv, ContainerImg, secretLabelv, RevSecImg, RevSecImg_, epoch, i, save_path):
    if not opt.debug:
        ...
        revSecFrames_ = RevSecImg_.resize_(this_batch_size, 3, opt.imageSize, opt.imageSize)

        showContainer = torch.cat([originalFrames, containerFrames], 0)
        showReveal = torch.cat([secretFrames, revSecFrames, revSecFrames_], 0)
